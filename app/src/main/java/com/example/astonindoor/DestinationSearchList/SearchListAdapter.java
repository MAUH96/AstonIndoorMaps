package com.example.astonindoor.DestinationSearchList;
...
public class SearchListAdapter extends RecyclerView.Adapter<SearchListAdapter.SearchViewHolder> implements Filterable {
    private List<RoomModel> listOfRooms = new ArrayList<>(); //need to be changed
    private List<RoomModel> copyListOfRooms; //this copy will have all the items
    ...
    public void setListOfRooms(List<RoomModel> listOfRooms) {
    ...
    public void onBindViewHolder(@NonNull SearchViewHolder holder, int position) {
        RoomModel currentItem = listOfRooms.get(position);
    ...
    private Filter listFilter = new Filter() {
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            List<RoomModel> filteredList = new ArrayList<>(); // contains filtered items
            ...
            if (constraint == null || constraint.length() == 0) { // if input field is empty
                ...
                for (RoomModel item : copyListOfRooms) {
